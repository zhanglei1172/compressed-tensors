...
def quantize_ste(
    ...
    :param args: quantization args dictating how to quantize_ste x
...
def dequantize_ste(
    ...
    :param scale: scale tensor
    :param zero_point: zero point tensor
    :param args: quantization args used to quantize_ste x_q
...
def fake_quantize_ste(
    ...
    """
    Fake quantize_ste the input tensor x by quantizing then dequantizing with
...
def _process_quantization(
    ...
    # blockwise FP8: quantize_ste per 2D block, supports block_structure for static block
    # quantization
    if args.strategy == QuantizationStrategy.BLOCK:
        ...
        if do_dequantize:
            # dequantize_ste blocks
            x_blocks = _dequantize_ste(
                x_q=x_blocks,
                scale=sb,
                zero_point=zb,
                global_scale=global_scale,
...
def patch_attr(base: Union[object, type], attr: str, value: Any):
    _sentinel = object()
    target_obj = base if isinstance(base, type) else type(base)
    original_value = getattr(target_obj, attr, getattr(base, attr, _sentinel))
    is_property = False
    if isinstance(original_value, property):
        is_property = True
        delattr(target_obj, attr)
    else:
        delattr(base, attr)
    ...
    try:
        ...
        delattr(base, attr)
        if is_property:
            # Restore the original property object to the class
            setattr(target_obj, attr, original_value)
        elif original_value is not _sentinel:
            # Restore the original non-property attribute/method
            setattr(base, attr, original_value)

def wrap_module_forward_quantized_ste(module: Module, scheme: QuantizationScheme):
    ...
    def wrapped_forward(self, *args, **kwargs):
        ...
        if scheme.input_activations is not None:
            ...
            input_ = forward_quantize_ste(self, input_, "input", scheme.input_activations)

        if scheme.weights is not None and not compressed:
            # calibrate and (fake) quantize_ste weights when applicable
            qdq_weight = forward_quantize_ste(
                self, self.weight, "weight", scheme.weights
            )
            with patch_attr(self, "weight", qdq_weight):
        ...
        if scheme.output_activations is not None:
            # forward-hook should calibrate/forward_quantize_ste
            if (
            ...
            output = forward_quantize_ste(
                self, output, "output", scheme.output_activations
            )
        return output
...
def forward_quantize_ste(
    ...
    if args.dynamic in (True, DynamicType.LOCAL):
        ...
        if base_name == "weight":
            value = value.reshape(1, ori_shape[0], -1)
...
def _quantize_ste(
    ...
    quantized_value = round_to_quantized_type_args(
        tensor=scaled, args=args, min=q_min, max=q_max, ste=True
    )
